"""SMTP email delivery for Covet alert notifications."""
...
def send_alert_digest(
    *,
    ...
    subject = f"Covet — {len(alerts)} alert{'s' if len(alerts) != 1 else ''} need attention"
...
def _render_text(username: str, alerts: list[dict]) -> str:
    ...
    for a in alerts:
        ...
        if a.get("details"):
            line += f"\n    {a['details']}"
        lines.append(line)
    lines += ["", "View your full maintenance agenda at your Covet instance.", ""]
    return "\n".join(lines)
...
<h2>Covet — alerts for {username}</h2>
...
You are receiving this because you have alert notifications enabled in Covet settings.
</p>
</body>
</html>"""
