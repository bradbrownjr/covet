...
users can wire Covet into their HA dashboards and automations without
...
from covet.models import Collection, Item
...
def ha_sensors(
    ...
    - ``sensor.covet_<slug>_item_count`` — total active item count
    - ``sensor.covet_<slug>_low_stock_count`` — items below minimum_quantity
    - ``sensor.covet_<slug>_overdue_alerts`` — alerts past due date
    - ``sensor.covet_<slug>_due_soon_alerts`` — alerts within 7 days
    ...
    sensor:
      - platform: rest
        resource: https://your-covet-server/api/ha/sensors
        headers:
...
def ha_blueprint(
    ...
    """Download a Home Assistant automation blueprint for Covet alerts.
    ...
    return textwrap.dedent("""
        blueprint:
          name: Covet — Alert Notification
          description: >
            Sends a persistent notification when a Covet collection has overdue
            or due-soon alerts. Requires a REST sensor named
            `sensor.covet_<collection>_overdue_alerts` configured to poll your
            Covet server.
          domain: automation
          input:
            overdue_sensor:
              ...
              description: >
                The `sensor.covet_*_overdue_alerts` entity for your collection.
              selector:
                entity:
                  integration: rest
            due_soon_sensor:
              ...
              description: >
                The `sensor.covet_*_due_soon_alerts` entity for your collection.
              selector:
                entity:
                  integration: rest
            notify_target:
