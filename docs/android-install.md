# Installing Covet on Android

Covet is distributed as an APK (Android application package). It is not
...
3. Paste the Covet repository URL:
   ```
   https://github.com/bradbrownjr/covet
   ```
4. Under **APK filter**, enter `covet-` so Obtanium only considers the Covet
...
its settings). Covet will appear in the list when a new version is
...
1. Go to the [Covet releases page](https://github.com/bradbrownjr/covet/releases).
2. Under the latest release, expand **Assets** and download `covet-X.Y.Z.apk`
...
Covet will request the following permissions on first use:

| Permission | Used for |
|---|---|
| **Internet** | Connecting to your Covet server |
...
granted later in **Android Settings → Apps → Covet → Permissions**.
...
| Obtanium shows two APK options | Choose the file named `covet-X.Y.Z.apk`; ignore any file with `-release-unsigned` in the name. |
