package io.github.bradbrownjr.covet.nfc
...
/**
 * Singleton that manages NFC tag reading and writing for Covet item tags.
 *
 * Tag format: a URI NDEF record containing `covet://item/<item-id>`.
 *
 * Usage:
...
object NfcManager {
    ...
    private const val COVET_SCHEME = "covet"
    private const val COVET_HOST = "item"
    ...
    /**
     * Item IDs read from a tapped Covet NFC tag. The nav layer should
    ...
    private fun writeNdef(tag: Tag, itemId: String) {
        try {
            val record = NdefRecord.createUri("$COVET_SCHEME://$COVET_HOST/$itemId")
    ...
    private fun readNdef(intent: Intent): Boolean {
        ...
        for (record in first.records) {
            ...
            if (uri.scheme == COVET_SCHEME && uri.host == COVET_HOST) {
